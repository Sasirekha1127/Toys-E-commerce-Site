...
export default function Home() {
  ...
  const filteredSoft = filterProducts(softToysList);
  const filteredEdu = filterProducts(educationalToysList);
  const filteredElec = filterProducts(electronicToysList);
  const filteredWood = filterProducts(woodenToysList);

  const hasAnyResults = filteredSoft.length > 0 || filteredEdu.length > 0 || filteredElec.length > 0 || filteredWood.length > 0;
  ...
  return (
    <div className="page-enter">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        {!hasAnyResults && (searchTerm || selectedCategory !== 'All Categories') ? (
          <div className="text-center py-20 bg-white rounded-3xl border-2 border-dashed border-orange-200 my-10">
            <div className="text-6xl mb-4">🔍</div>
            <h3 className="text-2xl font-bold text-gray-800 mb-2">No toys found</h3>
            <p className="text-gray-500">We couldn't find any toys matching "{searchTerm}" in {selectedCategory}.</p>
            <button 
              onClick={() => { window.location.reload(); }}
              className="mt-6 px-6 py-2 rounded-xl bg-orange-500 text-white font-bold hover:bg-orange-600 transition-all"
            >
              Clear Filters
            </button>
          </div>
          ...
          <>
            {filteredSoft.length > 0 && (
              <div id="soft-toys" className="mb-6">
                ...
                />
                <ProductCarousel
                  products={filteredSoft.slice(0, 10)}
                  color="orange"
                />
              </div>
            )}
            ...
            {filteredEdu.length > 0 && (
              <div id="educational-toys" className="mb-6">
                ...
                />
                <ProductCarousel
                  products={filteredEdu.slice(0, 10)}
                  color="orange"
                />
              </div>
            )}
            ...
            {filteredElec.length > 0 && (
              <div id="electronic-toys" className="mb-6">
                ...
                />
                <ProductCarousel
                  products={filteredElec.slice(0, 10)}
                  color="orange"
                />
              </div>
            )}

            {filteredWood.length > 0 && (
              <div id="wooden-toys" className="mb-6">
                ...
                />
                <ProductCarousel
                  products={filteredWood.slice(0, 10)}
                  color="orange"
                />
              </div>
            )}
