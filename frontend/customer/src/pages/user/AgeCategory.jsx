...
export default function AgeCategory() {
  ...
  const softToysList = softToys.map(mapProduct);
  const educationalToysList = educationalToys.map(mapProduct);
  const electronicToysList = electronicToys.map(mapProduct);
  const woodenToysList = woodenToys.map(mapProduct);
