...
export default function Vendors() {
  ...
  const verifiedCount = adminVendors.filter(v => v.status === 'active').length;
  const totalProducts = adminVendors.reduce((sum, v) => sum + Number(v.total_products || 0), 0);
          ...
          adminVendors.map((vendor) => (
            <div key={vendor.seller_id} className="card p-5 flex flex-col group hover:-translate-y-1 transition-all duration-300">
              <div className="flex justify-between items-start mb-4">
                ...
                <StatusBadge status={vendor.status ? vendor.status.charAt(0).toUpperCase() + vendor.status.slice(1) : 'Pending'} />
              </div>
              ...
              <div className="mt-auto relative">
                {editingVendorId === vendor.seller_id ? (
                  <select 
                    autoFocus
                    value={vendor.status || 'pending'}
                    onChange={(e) => {
                      updateVendorStatus(vendor.seller_id, e.target.value);
                      setEditingVendorId(null);
                    }}
                    onBlur={() => setTimeout(() => setEditingVendorId(null), 150)}
                    className="w-full py-2.5 px-3 rounded-xl text-xs font-bold transition-all shadow-sm outline-none border border-brand-200 bg-white text-gray-800 cursor-pointer"
                  >
                  ...
                  <button 
                    onClick={() => setEditingVendorId(vendor.seller_id)}
                    className="w-full py-2.5 rounded-xl text-xs font-bold transition-all shadow-sm bg-gray-50 text-gray-700 border border-gray-200 hover:bg-gray-100 active:scale-95"
                  >
                    Change Status
                  </button>
                )}
