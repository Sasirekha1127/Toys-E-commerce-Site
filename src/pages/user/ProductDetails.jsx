...
const mapProductData = (raw) => {
  ...
  const mainImageRaw =
    ...
    raw.imageUrl ||
    raw.image_url ||
    (raw.variants && raw.variants.length > 0 && raw.variants[0].image_url) ||
    raw.thumbnail ||
    raw.images?.[0] ||
    raw.image_urls?.[0] ||
    raw.gallery?.[0] ||
    ...
    const fetchProductDetails = async () => {
      ...
      try {
        ...
        if (res.ok) {
          const data = await res.json();
          if (data?.product) {
            const mapped = mapProductData(data.product);
            applyProduct(mapped);

            try {
              const relRes = await fetch(`${BASE_URL}/api/products`);
              if (relRes.ok) {
                const relData = await relRes.json();
                if (relData?.products) {
                  const dbRelated = relData.products
                    .filter(p =>
                      p.category === mapped.category &&
              ...
              console.warn('[ProductDetails] Related fetch error:', err);
              setRelated([]);
            }
          } else {
            setProduct(null);
        ...
        console.warn('[ProductDetails] API fetch error:', err);
        setProduct(null);
        setRelated([]);
      }
            ...
            >
              {displayImage ? (
                <img
                  ...
                  onError={e => {
                    e.target.src = '/images/toy-placeholder.png';
                  }}

                />
              ) : (
                <div className={`w-full h-full flex items-center justify-center text-6xl ${product.gradient || 'toy-gradient-1'}`}>🧸</div>
              )}
        ...
        <div className="fixed inset-0 z-[999] bg-black/90 flex items-center justify-center p-6">
          ...
          <div className="w-full max-w-7xl grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
            <div className="bg-white rounded-2xl overflow-hidden flex items-center justify-center h-[70vh] cursor-crosshair" onMouseEnter={() => setShowZoomPreview(true)} onMouseLeave={() => setShowZoomPreview(false)} onMouseMove={handlePreviewMouseMove}>
              {displayImage && <img src={displayImage} alt={product.name} className="w-full h-full object-cover" onError={e => e.target.src = '/images/toy-placeholder.png'} />}
